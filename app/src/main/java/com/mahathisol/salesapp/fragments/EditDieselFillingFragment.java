...
public class EditDieselFillingFragment extends BottomSheetDialogFragment implements DatePickerDialog.OnDateSetListener {
    public static final String TAG = EditDieselFillingFragment.class.getSimpleName();
...
//        dialog = builder.create();
    ...
    public void doThis(EventDiesel eventDiesel) {



        id=eventDiesel.getDdata().getId();

        idamount.setText(eventDiesel.getDdata().getAmount());
        idrate.setText(eventDiesel.getDdata().getRate());
            idlitre.setText(eventDiesel.getDdata().getLitre());
        idbunk.setText(eventDiesel.getDdata().getBunk());
            idkm.setText(eventDiesel.getDdata().getKmreading());
         idremarks.setText(eventDiesel.getDdata().getRemarks());
        dateview.setText(eventDiesel.getDdata().getFilldate());
        routeSelid=eventDiesel.getDdata().getVehicleid();
...
//                                });
                                ...
                                for (Vehicleli.Vehiclelist  rv : rl)
                                {
                                    ...
                                    if(rv.getId().equals(routeSelid))  {
                                        idvehicle.setSelection(count);
                                        //   Toast.makeText(getActivity().getApplicationContext(), "delboySelid"+rv.getId(), Toast.LENGTH_SHORT).show();
                                    }
                                    count++;
                                }
    ...
    public void sendCreateCustomer(final String txtvehicle, final String txtfilldate, final String txtlitre,final String txtrate,  final String txtamount, final String txtbunk, final String txtremarks, final String txtkm ) {
        ...
        AndroidNetworking.post(ApiEndPoint.BASE_URL + ApiEndPoint.EDIT_DIESEL)
                    ...
                    public void onResponse(Response okHttpResponse, CreateDiesel cal) {
                        ...
                        if (!cal.getCreate().getType().equalsIgnoreCase("success")) {
                         ...
                         //   saveNameToLocalStorage(txtname, txtphoneNo, txtemailId, txtaddress1, txtaddress2, txtaddress3, txtpin, txtcity, txtstateid, txtgstinno, txtactive, NAME_SYNCED_WITH_SERVER);
                            progressActivity.showEmpty(emptyDrawable,
                                    "Failed To Update Diesel Filling Details",
                                    "Server Error");
                        } else {
...
//                                    "Please Assign Routes");
                            ...
                            progressActivity.showEmpty(succDrawable,
                                    "Diesel Filling Details Updated Successfully",
                                    "");
...
//        String routeSelid = null;
    ...
    private void setupToolbar() {
        toolbar.setTitle("Edit Diesel Filling");
