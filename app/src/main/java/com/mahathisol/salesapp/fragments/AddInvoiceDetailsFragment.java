...
public class AddInvoiceDetailsFragment extends BottomSheetDialogFragment implements OnItemSelectedListener, View.OnClickListener {

    int id=0;
    public static final String TAG = AddInvoiceDetailsFragment.class.getSimpleName();
...
//==========FUll Screen BS=========
        ...
        idinvoiceno.setText("Invoice No : " + route_counter + "-" + route_code);
        mSecurePrefs.edit().putString(SharedPrefKey.INVOICE_DETAILS_NO,invoice).apply();
        idcname.setText("Name : " + cname);
        idcmobile.setText(cmobile);
        idcgstinno.setText("GSTIN No : " + cgst);
        idccity.setText(ccity);
        idinvdate.setText("Date : " + cinvoicedate);
    ...
    public void save() {
        ...
        if (!validate()) {
            ...
            int invoiceid = db.getLastInvoiceId("Invoice");
        mSecurePrefs.edit().putString(SharedPrefKey.PAY_AMOUNT, idtotalamt.getText().toString()).apply();
        mSecurePrefs.edit().putString(SharedPrefKey.INVOICEID_TO_PRINT, String.valueOf(invoiceid)).apply();
            int count = db.checkRecord(pr_typeid, pr_variantid, String.valueOf(invoiceid));
            if (count > 0) {
                new MaterialDialog.Builder(this.getActivity())
                        .iconRes(R.drawable.iconhems)
                        .limitIconToDefaultSize()
